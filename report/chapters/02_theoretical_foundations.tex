...
\subsection{Non-Contrast Computed Tomography (NCCT)}

NCCT imaging is the first-line imaging modality in acute stroke care due to several advantages:
...
\subsection{Computed Tomography Angiography (CTA)}
\begin{itemize}
    \item CT imaging with contrast agents to visualize vascular structures and detect pathologies.
\end{itemize}

\subsection{Computed Tomography Perfusion (CTP)}

\begin{itemize}
    \item Dynamic CT method quantifying brain perfusion through temporal contrast distribution analysis.
    \item Derived perfusion maps:
    \begin{itemize}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.3\textwidth}
        \includegraphics[width=\textwidth,height=4cm,keepaspectratio]{figures/ncct2.png}
        \caption{Non-Contrast CT image (NCCT)}
        \label{fig:ncct}
    \end{subfigure}
    \caption{Multimodal stroke imaging data}
...
The data used in this project is from two clinical centers: Munich and Zurich. This limited availability of data from multiple centers reduces the variability of deconvolution algorithms and thresholds used in the estimation of core and penumbra volumes.

\section{Lesion Masks and Ground Truth}

Lesion masks represent binary infarct masks in the dataset and are derived from MRI images \cite{riedel2024isles}. They show the actual final infarct areas and are co-registered to the Non-Contrast CT (NCCT). These masks serve as ground truth—the target that predictive models aim to accurately reproduce. For the ISLES'24 dataset, this ground tru th was generated using a hybrid human-algorithm approach: first, a robust deep-learning ensemble model from the ISLES'22 challenge \cite{delarosa2024robust} was used to create initial segmentations, which were then visually inspected and manually revised by neuroradiologists.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/lesion_mask_ground_truth/without_mask.png}
        \caption{NCCT without lesion mask}
        \label{fig:ncct_without_mask}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/lesion_mask_ground_truth/with_mask.png}
        \caption{NCCT with lesion mask overlay}
        \label{fig:ncct_with_mask}
    \end{subfigure}
    \caption{Comparison of a Non-Contrast CT (NCCT) image without (left) and with (right) lesion mask overlay. The lesion mask highlights the final infarct area and serves as ground truth for predictive models.}
    \label{fig:lesion_mask_comparison}
\end{figure}